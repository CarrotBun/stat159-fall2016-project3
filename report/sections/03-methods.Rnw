...
###Standardization  
The dataset includes categories measured in different scales such as admissions rate and age (which are on very different scales). In addition, to prevent any biased weighting, we want to standardize the dataset before building any models upon it.  

First, we create inidicator variables for the qualitative variables discussed in the introduction. Next, we want to standardize the absolute quantitites to relative quantities (mean centering). This means that each variable will have mean zero, and standard deviation one.  
